...
//!
//! NanoChronometer patch: runtime detection is enabled on `target_os =
//! "none"`. Upstream returns `false` there because a freestanding binary
//! cannot know whether its environment enabled the SSE/AVX state; the
//! nanochrono-baremetal kernel *is* that environment, and its boot stub sets
//! CR4.OSFXSR/OSXMMEXCPT/OSXSAVE and XCR0 before any Rust runs, so CPUID's
//! answer (and XGETBV's, for the wide register files) is the truth.
//!
